		...
		for(uint8_t j = 0; j < COL; j++) {
			if(bricks[i][j].available) {
				...
				if(((ball.x >= bricks[i][j].coord.x && ball.x <= bricks[i][j].coord.x + BRICK_WIDTH - 1) || (ball.x + BALL_WIDTH - 1 >= bricks[i][j].coord.x && ball.x + BALL_WIDTH - 1 <= bricks[i][j].coord.x + BRICK_WIDTH - 1))
						&& ball.y - 1 == bricks[i][j].coord.y + BRICK_HEIGHT - 1) {
					if(dir == UP_LEFT)
						dir = DOWN_LEFT;
					else
						dir = DOWN_RIGHT;
				...
				// Left hit
						 ...
						 && ball.x - 1 == bricks[i][j].coord.x + BRICK_WIDTH - 1) {
					if(dir == UP_LEFT)
						dir = UP_RIGHT;
					else
						dir = DOWN_RIGHT;
				...
				// Right hit
						 ...
						 && ball.x + BALL_WIDTH == bricks[i][j].coord.x) {
					if(dir == UP_RIGHT)
						dir = UP_LEFT;
					else
						dir = DOWN_LEFT;
				...
				// Lower hit
						...
						&& ball.y + BALL_HEIGHT == bricks[i][j].coord.y) {
					if(dir == DOWN_LEFT)
						dir = UP_LEFT;
					else
						dir = UP_RIGHT;
